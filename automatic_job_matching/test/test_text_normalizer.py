...
class TextNormalizationTestCase(TestCase):
	...
	def test_extract_possible_code_edge_cases(self):
		"""Test _extract_possible_code with edge cases."""
		from automatic_job_matching.utils.text_normalizer import _extract_possible_code
		
		# Test with very short string
		result = _extract_possible_code("ab")
		self.assertIsNone(result)
		
		# Test with string that has no alphanumeric
		result = _extract_possible_code("!!!")
		self.assertIsNone(result)
		
		# Test with valid code pattern
		result = _extract_possible_code("A.4.4.3.53 some description")
		self.assertIsNotNone(result)
		
		# Test with mixed case
		result = _extract_possible_code("X.01.02.03")
		self.assertIsNotNone(result)

	def test_normalize_text_with_preserve_code_true(self):
		"""Test normalize_text with preserve_code=True."""
		from automatic_job_matching.utils.text_normalizer import normalize_text
		
		# Should preserve code format
		result = normalize_text("A.4.4.3.53 pemasangan keramik", preserve_code=True)
		self.assertIn("a", result.lower())
		
		# Test with no code pattern
		result = normalize_text("pemasangan keramik tanpa kode", preserve_code=True)
		self.assertIn("pemasangan", result)

	def test_normalize_text_with_preserve_code_false(self):
		"""Test normalize_text with preserve_code=False (default)."""
		from automatic_job_matching.utils.text_normalizer import normalize_text
		
		# Should not specially handle codes
		result = normalize_text("A.4.4.3.53 pemasangan keramik", preserve_code=False)
		self.assertIn("pemasangan", result)

	def test_normalize_text_empty_after_code_extraction(self):
		"""Test when text is empty after code extraction."""
		from automatic_job_matching.utils.text_normalizer import normalize_text
		
		# Just a code with no description
		result = normalize_text("A.4.4.3.53", preserve_code=True)
		self.assertIsNotNone(result)

	def test_looks_like_code_patterns(self):
		"""Test _looks_like_code function."""
		from automatic_job_matching.utils.text_normalizer import _looks_like_code
		
		# Valid codes
		self.assertTrue(_looks_like_code("A.4.4"))
		self.assertTrue(_looks_like_code("X.01.02"))
		self.assertTrue(_looks_like_code("B123"))
		
		# Invalid codes
		self.assertFalse(_looks_like_code("abc"))
		self.assertFalse(_looks_like_code("123"))
		self.assertFalse(_looks_like_code("a"))

	def test_extract_code_segments(self):
		"""Test _extract_code_segments function."""
		from automatic_job_matching.utils.text_normalizer import _extract_code_segments
		
		# Test with dot-separated code
		result = _extract_code_segments("A.4.4.3.53")
		self.assertIsInstance(result, list)
		self.assertGreater(len(result), 0)
		
		# Test with alphanumeric code
		result = _extract_code_segments("ABC123XYZ")
		self.assertIsInstance(result, list)

	def test_normalize_text_preserve_decimal_with_code(self):
		"""Test decimal preservation with code patterns."""
		from automatic_job_matching.utils.text_normalizer import normalize_text
		
		# Decimal in description
		result = normalize_text("A.01 beton k225 1.5m", preserve_decimal=True, preserve_code=True)
