        ...
        if (descCell && descCell.textContent.trim() === itemName) {
          ...
          if (warnHost) {
            ...
            if (!warnHost.querySelector('.warning-chip-not-in-db')) {
              ...
              chip.innerHTML = `
                <span class="dot"></span>
                <span>Tidak ada di database - harga manual diperlukan</span>
              `;
      ...
      rows.forEach(row => {
        ...
        if (descCell && descCell.textContent.includes(itemName)) {
          ...
          if (warnHost) {
            ...
            chip.innerHTML = `
              <span class="dot"></span>
              <span>Harga ${deviation.deviation_percentage > 0 ? 'lebih tinggi' : 'lebih rendah'} dari referensi (${deviation.deviation_percentage > 0 ? '+' : ''}${deviation.deviation_percentage.toFixed(1)}%)</span>
            `;
            warnHost.appendChild(chip);
          }
        }
